...
import net.mulaudzi.service.MovieCatalogService;
...
public class MovieCatalogController {
	...
	public List<Movie> getCatalog(@PathVariable String userId) {
		 ...
		 */
		List<Movie> movies = movieCatalogService.getCatalog(userId);
		/*
		...
		for(int idx = 0; idx < movies.size(); idx++) {
			...
			Movie movieRating = restTemplate.getForObject("http://movie-ratings-service/"+"ratings/"+userId+"/"+movies.get(idx).getTitle(), Movie.class);
			movies.set(idx, new Movie(movieInfo.getTitle(),movieInfo.getDescirption(), movieRating.getRating()));
		}		
